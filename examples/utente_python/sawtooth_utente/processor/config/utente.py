...
def load_default_utente_config():
    """
    Returns the default utente
    """
    return UtenteConfig(
...
def load_toml_utente_config(filename):
    ...
    Returns:
        config (UtenteConfig): The UtenteConfig created from the stored
            toml file.
...
def merge_utente_config(configs):
    """
    Given a list of UtenteConfig objects, merges them into a single
    UtenteConfig, giving priority in the order of the configs
    (first has highest priority).

    Args:
        config (list of UtenteConfig): The list of xo configs that
            must be merged together

    Returns:
        config (UtenteConfig): One XOConfig that combines all of the
            passed in configs.
    """
    connect = None
